...
# from flaskconndb importconn
# WTForms is a flexible forms validation and rendering library for Python web developmentpip install -U WTFormspip install -U WTForms
...
conn =get_db_connection()#calling db conn handeller
...
def login():
	if request.method == 'POST':
		...
		data = cur.fetchone()
		print("inputNAme:",username," inputPass:",password_candidate,"\nresult ",data)
		if data:			
			password = data[1]
...
def update_password(username):
	...
	if request.method == 'POST' and form.validate():
		...
		if sha256_crypt.verify(entered, old):
			cur.execute("UPDATE info SET password = %s WHERE username = %s", (sha256_crypt.hash(new), username))
			conn.commit()
			cur.close()
...
def addTrainor():
	values.clear()
	cur =conn.cursor()
	cur.execute("SELECT username,street FROM info")
	b = cur.fetchall()
	# print(len(b))
...
def trainorDash():
	...
	if request.method == 'POST':
		...
		if datetime.now().date()<date:
			flash('You cannot predict furture!!', 'warning')
			choices.clear()
			return redirect(url_for('trainorDash'))
...
def memberDash(username):
	...
	plan = cur.fetchall()
	print(plan[0])
	cur.execute("SELECT exercise, reps, sets FROM plans WHERE name = %s", [plan[0]])
	scheme = cur.fetchall()
	print(scheme)
	...
	for i in range(len(progress)):
		result.append(int(progress[i][2]))
		# print(progress[i][2])
	good = result.count(1)
	poor = result.count(3)
	...
	# return render_template('memberDash.html',user = username, scheme = scheme)# plan = plan, progress = progress, good = good, poor = poor, average = average
